...
def main(args: argparse.Namespace):
    ...
    for epoch in range(args.epochs):
        ...
        dot_prod_train = train_data @ weights
        pred_train = (sigmoid(dot_prod_train))
        dot_prod_test = test_data @ weights
        pred_test = (sigmoid(dot_prod_test))
        ...
        train_log_loss, test_log_loss = log_loss(train_target, pred_train), log_loss(
            test_target, pred_test
        )

        print(
            f"Epoch {epoch+1}: train loss {train_log_loss:.6f} acc {train_accuracy*100:.2f}, test loss {test_log_loss:.6f} acc {test_accuracy*100:.2f}"
        )
